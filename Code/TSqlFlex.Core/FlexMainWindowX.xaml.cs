...
namespace TSqlFlex.Core
{
    ...
    public partial class FlexMainWindowX : System.Windows.Controls.UserControl
    {
        ...
        private void SetConnectionText()
        {
            ...
            {
                lblConnectionInfo.Content = currentConnectionText();
            });
        ...
        private void setProgressText(bool isComplete)
        {
            InvokeFireAndForgetOnFormThread(() =>
            {
                lblProgress.Content = getFormattedElapsedSqlStopwatchTime(isComplete) + " " + progressText;
            });
        }
        ...
        private void TryToSaveCSVs(SqlRunParameters srp)
        {
            ...
            for (int csvIndex = 0; csvIndex < srp.outputFiles.Count; csvIndex += 1)
            {
                ...
                try
                {
                    fileName = System.IO.Path.Combine(personalFolder, "TSqlFlex" + timestamp +
        ...
        private void TryToSaveSpreadsheet(SqlRunParameters srp)
        {
            ...
            try
            {
                fileName = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), "TSqlFlex" + DateTime.Now.ToString("_yyyyMMddTHHmmss") + ".xml");
            }
            catch (Exception ex)
            {
                string error = "Exception when attempting to find personal documents folder for current user.  Will not save file.";
        ...
        private void setUIState(bool queryIsRunning)
        {
            ...
            cmdCancel.IsEnabled = queryIsRunning;
            cmdRunNRollback.IsEnabled = !queryIsRunning;
            //if (queryIsRunning)
            //{
            //    Cursor.Current = Cursors.WaitCursor;
            //}
            ...
            //}
            btnExcel.IsEnabled = (lastExportedFilePath.Length > 0);
            btnTxt.IsEnabled = (lastExportedFilePath.Length > 0);
        ...
        private void btnAbout_Click(object sender, EventArgs e)
        {
            ...
            //ab.ShowDialog();
            MessageBox.Show("Placeholder About box.");
        }
